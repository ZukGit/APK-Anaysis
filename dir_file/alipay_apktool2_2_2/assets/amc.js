...
// 兼容收银台H5容器
amc.isH5 = !!window.MQPJSBridge;
if (amc.isH5) {
    // rpcData
    window.flybird = {
        rpcData: window.MQPJSBridge.startParams.rpcData
    };
    
    if(window.MQPJSBridge && window.MQPJSBridge.startParams.local) {
        var u = navigator.userAgent;
        var isIOS = !!u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/);
        MQPJSBridge.startParams.local.platform = isIOS ? 'iOS' : 'android';
    }

    // 启动参数
    document.getCustomAttr = function(name) {
        try {
            return window.MQPJSBridge.startParams.local[name];
        } catch (e) {
            return null;
        }
    }
    window.innerHeight = (screen && screen.height) || window.innerHeight;
    window.innerWidth = (screen && screen.width) || window.innerWidth;
}

// 平台信息
amc.platform = amc.isH5 ? document.getCustomAttr('platform') : (document.platform || 'chrome');
amc.isIOS = (amc.platform === 'iOS');
amc.isAndroid = (amc.platform === 'android');
amc.path = amc.isH5 ? 'https://amc201710.alipay.com/' : (amc.isAndroid ? 'com.alipay.android.app/' : 'AlipaySDK.bundle/');

// 兼容收银台H5容器-end

amc.rpcData = flybird.rpcData || {};
amc.isFlybird = (amc.isIOS || amc.isAndroid || amc.isH5);
...
amc.fn.logAction = function(action, actionType) {
    ...
    paramObj['type'] = actionType ? (actionType + '') : 'tpl-click';
 ...
 */
amc.fn.mqpSpm = function(type, actionId, spmId, param4Map, ABTestId) {
    if (!type || !spmId) {
        return;
    }
 ...
 * params: 扩展字段
 */
amc.fn.spmClickServer = function(feedback) {
    if (!feedback || !feedback['spmId']) {
        return;
    }

    var params = {};
    for (var key in feedback) {
        if (key == 'ABTestId' || key == 'spmId') {
            continue;
        }
        params[key] = feedback[key];
    }

    amc.fn.mqpSpm('click', 'clicked', feedback['spmId'], params, feedback['ABTestId']);
}
 ...
 * params: 扩展字段
 */
amc.fn.spmExposureServer = function(feedback) {
    if (!feedback || !feedback['spmId']) {
        return;
    }
    ...
    amc.fn.mqpSpm('exposure', 'exposure', feedback['spmId'], params, feedback['ABTestId']);
}
